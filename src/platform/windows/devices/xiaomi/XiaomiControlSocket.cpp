...
bool ConnectForService(std::uint64_t bluetooth_address,
    ...
    const auto try_connect = [&](const GUID& service_uuid, const char* service_name) -> bool {
        const auto connect_once = [&](bool secure_mode, const char* mode_suffix) -> bool {
            const SOCKET candidate_socket = socket(AF_BTH, SOCK_STREAM, BTHPROTO_RFCOMM);
            if (candidate_socket == INVALID_SOCKET) return false;
    ...
    return service == ClassicBatteryService::kZmiPurPodsSerial
...
bool ConnectXiaomiControlSocket(std::uint64_t bluetooth_address,
    ...
    *socket_handle = INVALID_SOCKET;
    ...
    if ((!preferred.has_value() || preferred_service != *preferred) && try_service(preferred_service)) {
        return true;
    }
    ...
    for (const auto service : services) {
        if ((!preferred.has_value() || service != *preferred) && service != preferred_service &&
            try_service(service)) {
            return true;
        }
    }
