...
bool IsLikelyZmiPurPods(const std::string& primary_name,
                        const std::string& secondary_name,
                        const std::string& device_id) {
    const std::string probe = ToLowerAscii(primary_name + " " + secondary_name + " " + device_id);
...
std::optional<std::uint8_t> TryReadGenericClassicHfpBatteryAdapter(std::uint64_t bluetooth_address) {
    return TryReadGenericClassicHfpBattery(
        bluetooth_address,
        GetWindowsBatteryProviderRuntimeOptions().debug_enabled,
        &WindowsBatteryProviderDebugLog);
}
...
const WindowsBatteryProviderRuntimeOptions& GetWindowsBatteryProviderRuntimeOptions() {
    static const WindowsBatteryProviderRuntimeOptions options = []() {
        ...
        value.xiaomi_advertisement_scan_ms =
            ReadClampedIntEnv("BATTERY_MONITOR_XIAOMI_ADV_SCAN_MS", 1800, 300, 6000);
        value.zmi_observe_ms = ReadClampedIntEnv("BATTERY_MONITOR_ZMI_OBSERVE_MS", 0, 0, 30000);
...
WindowsBatteryQueryReaderContext MakeWindowsBatteryQueryReaderContext() {
    ...
    return WindowsBatteryQueryReaderContext{
        ...
        .is_likely_zmi_purpods = &IsLikelyZmiPurPods,
        ...
        .read_phone_hfp_fallback = &TryReadGenericClassicHfpBatteryAdapter,
...
std::optional<ResolvedBluetoothTarget> ResolveConnectedZmiControlTarget(
    ...
    return ResolveBatteryTarget(
        ...
        [](const DeviceBatteryInfo& entry, const std::string& normalized_hint) {
            ...
            const std::string lowered_name = ToLowerAscii(entry.device_name);
            return lowered_name.find("zmi") != std::string::npos ||
                   lowered_name.find("purpods") != std::string::npos;
        });
}
