...
void CollectTwsCandidateBatteryEntries(const WindowsTwsCandidateBatteryCollectorContext& context,
    ...
    const auto fast_connected_entries = ReadConnectedBluetoothDeviceBatteryFast(query_reader_context, &tws_candidates);

    const bool should_scan_aep = context.force_aep_scan || tws_candidates.size() < 2U;
    if (should_scan_aep) {
        std::vector<EndpointCandidate> aep_tws_candidates;
        const auto endpoint_entries = ReadAssociationEndpointBattery(query_reader_context, &aep_tws_candidates);
    ...
    for (const auto& candidate : tws_candidates) {
        ...
        const bool likely_zmi_family =
            ...
            context.is_likely_zmi_purpods(candidate.endpoint_name, candidate.endpoint_name, candidate.endpoint_id);
        ...
        if (!maybe_ble_device.has_value()) {
            ...
            if (likely_xiaomi_tws) {
                const auto advertisement_readings =
                    xiaomi_advertisement_cache->Read(candidate.bluetooth_address,
                                                     candidate.endpoint_name,
                                                     likely_zmi_family);
                if (advertisement_readings.size() >= 2U) {
                    xiaomi_classic_cache->Persist(candidate.bluetooth_address, advertisement_readings);
                    AddCandidateReadings(accumulator, candidate, advertisement_readings, false, candidate.is_connected);
                    LogDebug(context.debug_log,
                             "AEP advertisement fallback used without BLE open for '" +
                                 ResolveCandidateDeviceName(candidate) + "'");
                    continue;
                }
            }
        ...
        if (likely_xiaomi_tws && !HasUsefulXiaomiTwsReadings(resolved_readings)) {
            ...
            if (advertisement_readings.size() >= 2U) {
                ...
                resolved_from_persistent_cache = false;
                LogDebug(context.debug_log,
                         "AEP advertisement fallback accepted for '" + candidate.endpoint_name + "'");
            }
        }
